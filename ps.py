""" Parameter Server SGD Example """
# By: Gilles-Philippe Morin
# Based on [1], [2], [3], and [4]
# Usage: torchrun --nproc-per-node {number of processes} ps.py
...
# [4] https://github.com/LPD-EPFL/Garfield/tree/master/pytorch_impl/applications/Aggregathor
...
def aggregate_gradients(remote_parameters):
    """ Gradient aggregation. """
    global model
    global count
    for local_param, remote_param in zip(model.parameters(), remote_parameters):
        local_param.data = (local_param.data + remote_param.data * count) / (count + 1)
    count += 1
...
#            print("Before: ", [param for param in model.parameters()][0])
        remote_params = rpc.rpc_sync("parameter_server", aggregate_gradients, args=([param for param in model.parameters()],), timeout=1)
        for remote_param, local_param in zip(remote_params, model.parameters()):
            local_param.data.copy_(remote_param.data)
#        if rank == 1:
...
def run_server():
    ...
    rpc.init_rpc("parameter_server", rank=0, world_size=size)
    model = Net()
    model.zero_grad()
    rpc.shutdown()
...
def run_worker():
    ...
    sampler = DistributedSampler(train_set, num_replicas=size-1, rank=rank-1)
...
if __name__ == "__main__":
    ...
    processes = []
    if rank == 0:
        p = Process(target=init_process, args=(rank, size, run_server))
        p.start()
        processes.append(p)
    else:
